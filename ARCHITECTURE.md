...
| close | `Drop`: synchronous `close(2)`. `close_descriptor`: `IORING_OP_CLOSE`, ordered behind operations already submitted against the descriptor | `Drop`: synchronous `close(2)`. `close_descriptor`: the same synchronous close — kqueue has no asynchronous close, so only the `Closed`/`StillShared` outcome is gained, not the ordering | `Drop`: synchronous `CloseHandle`/`closesocket`. `close_descriptor`: the same synchronous close, outcome only |
